...
class Assets {
	...
	public function prefetch() {
		if ( Option::network_get( 'use_cdn' ) && MAILARCHIVER_CDN_AVAILABLE ) {
	...
	public function register_style( $handle, $src, $file, $deps = [], $media = 'all' ) {
		if ( Option::network_get( 'use_cdn' ) && MAILARCHIVER_CDN_AVAILABLE ) {
			if ( MAILARCHIVER_ADMIN_URL === $src ) {
				$file = 'https://cdn.jsdelivr.net/wp/' . MAILARCHIVER_SLUG . '/tags/' . MAILARCHIVER_VERSION . '/admin/' . $file;
			} else {
				$file = 'https://cdn.jsdelivr.net/wp/' . MAILARCHIVER_SLUG . '/tags/' . MAILARCHIVER_VERSION . '/public/' . $file;
			}
			// phpcs:ignore
