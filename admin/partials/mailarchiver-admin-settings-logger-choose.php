...
<div id="normal-sortables" class="meta-box-sortables ui-sortable" style="overflow: hidden;">
	<div class="postbox ">
		<h3 class="hndle" style="cursor:default;"><span><?php esc_html_e( 'Please, select the type of logger you want to add', 'mailarchiver' ); ?>&hellip;</span></h3>
		...
		jQuery(document).ready(function($) {
			...
			<?php foreach ( $logger_types->get_all() as $logger ) { ?>
				...
				$("#<?php echo $logger['id']; ?>").click(function() {
					<?php // phpcs:ignore ?>
					window.open('<?php echo add_query_arg( array( 'page'    => 'mailarchiver-settings', 'action'  => 'form-edit', 'tab'     => 'loggers', 'handler' => $logger['id'], ), admin_url( 'options-general.php' ) );?>', '_self');
				});
			<?php } ?>
		});
	</script>
