...
if ( ! class_exists( 'PerfOpsOne\AdminMenus' ) ) {
	class AdminMenus {
		...
		public static function initialize() {
			foreach ( apply_filters( 'init_perfops_admin_menus', [] ) as $menu => $submenus ) {
				if ( ! in_array( 'perfopsone-' . $menu, self::$slugs, true ) ) {
					switch ( $menu ) {
						case 'analytics':
							add_menu_page( esc_html__( 'Dashboard', 'mailarchiver' ), sprintf( esc_html__( '%s Analytics', 'mailarchiver' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_analytics_page' ], 'dashicons-chart-bar', 210874 );
							...
							break;
						case 'tools':
							add_menu_page( esc_html__( 'Available Tools', 'mailarchiver' ), sprintf( esc_html__( '%s Tools', 'mailarchiver' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_tools_page' ], 'dashicons-admin-tools', 210876 );
							...
							break;
						case 'insights':
							add_menu_page( esc_html__( 'Available Reports', 'mailarchiver' ), sprintf( esc_html__( '%s Insights', 'mailarchiver' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_insights_page' ], 'dashicons-lightbulb', 210875 );
							...
							break;
						case 'records':
							add_menu_page( esc_html__( 'Available Catalogues', 'mailarchiver' ), sprintf( esc_html__( '%s Records', 'mailarchiver' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_records_page' ], 'dashicons-book', 210873 );
							...
							break;
						case 'settings':
							add_menu_page( esc_html__( 'Control Center', 'mailarchiver' ), sprintf( esc_html__( '%s Settings', 'mailarchiver' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_settings_page' ], 'dashicons-admin-settings', 210871 );
							...
							break;
						case 'consoles':
							add_menu_page( esc_html__( 'Available Consoles', 'mailarchiver' ), sprintf( esc_html__( '%s Consoles', 'mailarchiver' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_consoles_page' ], 'dashicons-embed-generic', 210872 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Consoles', 'mailarchiver' ), __( 'Available Consoles', 'mailarchiver' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_consoles_page' ], 0 );
							break;
					}
		...
		private static function display_as_lines( $items ) {
			...
			foreach ( $items as $item ) {
				$links   = [];
				$links[] = '<a href="' . $item['url'] . '"/>' . __( 'settings', 'mailarchiver' ) . '</a>';
				$links[] = '<a href="' . $item['url'] . '&tab=about"/>' . __( 'about', 'mailarchiver' ) . '</a>';
				$links[] = '<a href="https://wordpress.org/support/plugin/' . $item['slug'] . '" target="_blank">' . __( 'support', 'mailarchiver' ) . '</a>';
				$links[] = '<a href="https://github.com/Pierre-Lannoy/wp-' . $item['slug'] . '" target="_blank">' . __( 'contribution', 'mailarchiver' ) . '</a>';
				$disp   .= '<div class="po-container">';
				$disp   .= ' <div class="po-actionable">';
